...
<?php $__env->startSection('nav-buttons'); ?>
<span>
    Menu:
                ...
                <div class="card-header">
                    <div class="row">
                        ...
                        <div class="col-md-4 text-right">
                            <?php if( (auth()->user()->id_unit_kerja ?? 0) == 1 ): ?>
...
$(function () {
    ...
    <?php $SuratMasuk = app('App\Http\Controllers\Surat\SuratMasukController'); ?>

    surat_asal_ekspedisi_surat_masuks = <?php echo json_encode($SuratMasuk::convertToJsVar($asal_ekspedisi_surat_masuks, 'asal_ekspedisi_surat_masuk')); ?>;
    surat_jenis_surat_masuks          = <?php echo json_encode($SuratMasuk::convertToJsVar($jenis_surat_masuks, 'jenis_surat_masuk')); ?>;
    surat_sifat_keamanan_surats       = <?php echo json_encode($SuratMasuk::convertToJsVar($sifat_keamanan_surats, 'sifat_keamanan_surat')); ?>

    surat_sifat_penyelesaian_surats   = <?php echo json_encode($SuratMasuk::convertToJsVar($sifat_penyelesaian_surats, 'sifat_penyelesaian_surat')); ?>

    surat_jenis_files                 = <?php echo json_encode($SuratMasuk::convertToJsVar($jenis_files, 'jenis_file')); ?>

    surat_arahan_surats               = <?php echo json_encode($SuratMasuk::convertToJsVar($arahan_surats, 'arahan_surat')); ?>

    surat_status_disposisis           = <?php echo json_encode($SuratMasuk::convertToJsVar($status_disposisis, 'status_disposisi')); ?>

    surat_jenis_disposisis            = <?php echo json_encode($SuratMasuk::convertToJsVar($jenis_disposisis, 'jenis_disposisi')); ?>

    surat_units                       = <?php echo json_encode($SuratMasuk::convertToJsVar($units, 'unit')); ?>
    ...
    surat_masuk_table = $('#surat_masuk-data-table').DataTable({
        ...
        responsive: true,
        order: [],
        ajax: "<?php echo e(route('surat_masuk.index')); ?>" + (statusMailbox != '' ? `?statusMailbox=${statusMailbox}&` : ''),
        columns: [
    ...
    function load_datatable_on_surat_masuk_id(surat_masuk_id) {
        ...
        surat_disposisi_pimpinan_table.ajax.url( '<?php echo e(route( 'disposisi_pimpinan.index' )); ?>?id_surat_masuk=' + surat_masuk_id ).load();
        surat_tembusan_surat_table.ajax.url( '<?php echo e(route( 'tembusan_surat.index' )); ?>?id_surat_masuk=' + surat_masuk_id ).load();
        surat_dokumen_surat_table.ajax.url( '<?php echo e(route( 'dokumen_surat.index' )); ?>?id_surat_masuk=' + surat_masuk_id ).load();
        surat_file_tanggapan_table.ajax.url( '<?php echo e(route( 'file_tanggapan.index' )); ?>?id_surat_masuk=' + surat_masuk_id ).load();
        surat_keluar_konsep_table.ajax.url( '<?php echo e(route( 'surat_keluar_konsep.index' )); ?>?id_surat_masuk=' + surat_masuk_id ).load();
    }
    ...
    $('body').on('click', '#surat_masuk-kirim-btn', function () {
        ...
        $.ajax({
            ...
            url: "<?php echo e(route('suratmasuk.kirimsurat')); ?>",
            type: "POST",
            dataType: 'json',
    ...
    viewSuratMasuk = function(surat_masuk_id) {
        $.get("<?php echo e(route('surat_masuk.index')); ?>" +'/' + surat_masuk_id +'/edit', function (data) {
            ...
            // Set mail box read
            $.get( '<?php echo e(route( 'suratmasuk.setmailboxread' )); ?>?id=' + id_mailbox , function(res) {
                surat_masuk_table.draw(false);
            });

            $.get( '<?php echo e(route( 'tembusan_surat.index' )); ?>?id_surat_masuk=' + surat_masuk_id , function(res) {
                html = '';
            ...
            $.get( '<?php echo e(route( 'dokumen_surat.index' )); ?>?id_surat_masuk=' + surat_masuk_id , function(res) {
                html = '';
                if ( res.data.length > 0 ) {
                    res.data.forEach(function(element, index){
                        if ( index == 0) {
                            var iframe = $('<iframe class="pdfobject" type="application/pdf" style="overflow: auto; width: 100%; height: 100%;">');
                            iframe.attr('src',"<?php echo e(url('/')); ?>/uploads/" + element.nama_file);
                            $('#surat-surat-masuk-preview-pdf').html(iframe);
                        }
                        html = html + "(" + (index + 1) + ") <a href=\"<?php echo e(url('/')); ?>/uploads/" + element.nama_file + "\" target=\"_blank\">" + element.dokumen_surat + "</a><br>";
                    });
                }
                $('#view-dokumen_surat', '#surat-surat-masuk-box-view').html(html);
            });

            $.get( '<?php echo e(route( 'disposisi_pimpinan.index' )); ?>?id_surat_masuk=' + surat_masuk_id , function(res) {
    ...
    $('body').on('change', '#nomor_surat', function () {
        ...
        if (nomor_surat.length > 0) {
            $.ajax({
                ...
                url: "<?php echo e(route('surat.ceknomorsurat')); ?>",
                type: "POST",
                dataType: 'json',
                success: function (data) {
                    // console.log('success', data);
    ...
    $('#surat_masuk-save-btn').click(function (e) {
        ...
        if (confirm('Apakah nomor surat yang di entri sudah sesuai dengan surat fisik?')) {
            ...
            $.ajax({
                ...
                url: "<?php echo e(route('surat_masuk.store')); ?>",
                type: "POST",
                dataType: 'json',
    ...
    $('body').on('click', '.surat_masuk-delete', function (){
        ...
        if(result){
            $.ajax({
                type: "DELETE",
                url: "<?php echo e(route('surat_masuk.store')); ?>"+'/'+surat_masuk_id,
                success: function (data) {
                    surat_masuk_table.draw();
                },
                error: function (data) {
    ...
    var reloadSuratMasukTable = function(statusMailbox) {
        surat_masuk_table.ajax.url(`<?php echo e(route('surat_masuk.index')); ?>?statusMailbox=${statusMailbox}&`);
        surat_masuk_table.draw();
...
<?php echo $__env->make('adminlte.layout', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\surat\resources\views/surat/surat-masuk/index.blade.php ENDPATH**/ ?>