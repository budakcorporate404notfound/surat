<div class="row mb-3">
    <div class="col-md-6" id="card_title">{{ Form::label('daftar_tembusan_surat') }}</div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="surat-tembusan-surat-form" name="surat-tembusan-surat-form"  method="POST"  role="form" enctype="multipart/form-data">
                    @csrf
                    {!! Form::hidden('id', '', ['id'=>'surat-tembusan-surat-id']) !!}
                    {!! Form::hidden('id_surat_masuk', '', ['id'=>'surat-tembusan-surat-id_surat_masuk']) !!}
...
$(function () {
    ...
    surat_tembusan_surat_table = $('#surat-tembusan-surat-data-table').DataTable({
        ...
        searching: false,
        info: false,
        ajax: "{{ route('tembusan_surat.index') }}",
        columns: [
    ...
    $('body').on('click', '.surat-tembusan-surat-edit', function () {
        ...
        $.get('{{ route('tembusan_surat.index') }}' + '/' + surat_tembusan_surat_id  + '/edit', function (data) {
            reset_form_data('#surat-tembusan-surat-form');
    ...
    $('#surat-tembusan-surat-save-btn').click(function (e) {
        ...
        $.ajax({
            ...
            url: "{{ route('tembusan_surat.store') }}",
            type: "POST",
            dataType: 'json',
    ...
    $('body').on('click', '.surat-tembusan-surat-delete', function (){
        ...
        if(result){
            $.ajax({
                type: "DELETE",
                url: '{{ route('tembusan_surat.store') }}' + '/' + surat_tembusan_surat_id,
                success: function (data) {
                    surat_tembusan_surat_table.ajax.url( '{{ route( 'tembusan_surat.index' ) }}?id_surat_masuk=' + surat_masuk_id ).load();
                },
                error: function (data) {
