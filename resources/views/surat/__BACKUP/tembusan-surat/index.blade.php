...
<div class="row mb-3">
    <div class="col-md-6" id="card_title">{{ Form::label('Tembusan Surat') }}</div>
...
<script type="text/javascript">
    ...
    $('body').on('click', '.tembusan_surat-edit', function () {
        ...
        $.get("{{ route('tembusan_surat.index') }}" +'/' + tembusan_surat_id +'/edit', function (data) {
    ...
    $('body').on('click', '.tembusan_surat-delete', function (){
        ...
        if(result){
            $.ajax({
                type: "DELETE",
                url: "{{ route('tembusan_surat.store') }}"+'/'+tembusan_surat_id,
                success: function (data) {
                    table.draw();
                },
                error: function (data) {
