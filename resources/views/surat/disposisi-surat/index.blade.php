<div id="surat-disposisi-surat-data-table-pos">
    <div id="surat-disposisi-surat-data-block">
        <div class="row mb-3">
            <div class="col-md-6" id="card_title">{{ Form::label('daftar_disposisi_surat') }}</div>
            <div class="col-md-6 text-right">
                {{-- && auth()->user()->id_unit_kerja != '1' --}}
                @if (auth()->user()->id_jabatan != '')
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="surat-disposisi-surat-form" name="surat-disposisi-surat-form"  method="POST"  role="form" enctype="multipart/form-data">
                    @csrf
                    {!! Form::hidden('id', '', ['id'=>'surat-disposisi-surat-id']) !!}
                    {!! Form::hidden('id_surat_masuk', '', ['id'=>'surat-disposisi-surat-id_surat_masuk']) !!}
                    {!! Form::hidden('id_mailbox', '', ['id'=>'surat-disposisi-surat-id_mailbox']) !!}
...
$(function () {
    ...
    $('#surat-disposisi-surat-create-new').click(function () {
        ...
        $.get('{{ route('surat.getalldisposisi') }}' + '/?id=' + $('#surat_masuk-form-id').val(), function (data) {
            console.log(data);
    ...
    $('body').on('click', '.surat-disposisi-surat-edit', function () {
        ...
        reset_form_data('#surat-disposisi-surat-form');
        $.get('{{ route('disposisi_surat.index') }}' + '/' + surat_disposisi_surat_id  + '/edit', function (data) {
    ...
    $('body').on('click', '.surat-disposisi-surat-delete', function (){
        ...
        if(result){
            $.ajax({
                type: "DELETE",
                url: '{{ route('disposisi_surat.store') }}' + '/' + surat_disposisi_surat_id,
                success: function (data) {
                    surat_disposisi_surat_table.ajax.url( '{{ route( 'disposisi_surat.index' ) }}?id_surat_masuk=' + surat_masuk_id ).load();
                },
                error: function (data) {
