        ...
        <div class="col-sm-12">
            <div class="card">
                <div class="card-header">
                    <div class="row">
                        <div class="col-md-6" id="card_title">{{ Form::label('daftar_konsep_balasan/jawaban_surat') }}</div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-footer text-right">
                ...
                <div class="input-group" style="width:50% !important;">
                    ...
                    {{-- @if ($arahan_surat->id == auth()->user()->id_unit_kerja) --}}
                    {{-- auth()->user()->id_unit_kerja --}}
                        <option value="{{ $arahan_surat->id }}"@if ($arahan_surat->id == $default_arahan) SELECTED @endif> {{ $arahan_surat->arahan_surat }}</option>
                    {{-- @endif --}}
                    @endforeach
                    </select>
...
$(function () {
    ...
    $('body').on('click', '.surat-surat_keluar_konsep-edit', function () {
        ...
        $.get("{{ route('surat_keluar_konsep.index') }}" +'/' + surat_keluar_konsep_id +'/edit', function (data) {
    ...
    $('body').on('click', '#surat-surat_keluar_konsep-kirim-btn', function () {
        ...
        $.ajax({
            ...
            url: "{{ route('suratkeluarkonsep.kirimdraft') }}/",
            type: "POST",
            dataType: 'json',
    ...
    $('#surat-surat_keluar_konsep-unduh-btn').click(function (e) {
        ...
        window.open("http://{{ Request::getHost() }}/surat/generatesurat/" + surat_keluar_konsep_id + '/' + surat_keluar_konsep_id_jenis_dokumen, '_blank');
    });
    ...
    $('#surat-surat_keluar_konsep-save-btn').click(function (e) {
        ...
        $.ajax({
            ...
            url: "{{ route('surat_keluar_konsep.store') }}",
            type: "POST",
            dataType: 'json',
            success: function (res) {
                if (res.errors) {
    ...
    $('body').on('click', '.surat_keluar_konsep-delete', function (){
        ...
        if(result){
            $.ajax({
                type: "DELETE",
                url: "{{ route('surat_keluar_konsep.store') }}"+'/'+surat_keluar_konsep_id,
