<div id="surat-diskusi-surat-data-table-pos">
    <div id="surat-diskusi-surat-data-block">
        <div class="row mb-3">
            <div class="col-md-6" id="card_title">{{ Form::label('daftar_diskusi_surat') }}</div>
                    ...
                    <table class="table table-bordered data-table dt-responsive" id="surat-diskusi-surat-data-table" width="100%">
                        ...
                        <tfoot>
                            <tr>
                                <td>
                                        @csrf
                                        {!! Form::hidden('id', '', ['id'=>'surat-diskusi-surat-id']) !!}
                                        {!! Form::hidden('id_surat_masuk', '', ['id'=>'surat-diskusi-surat-id_surat_masuk']) !!}
                                        @include('surat.diskusi-surat.form')
                                        {{-- <div class="col-sm-offset-2 col-sm-12 text-right">
                                            <button type="submit" class="btn btn-primary" id="surat-diskusi-surat-save-btn" value="create">Simpan perubahan</button>
                                            <button type="button" class="btn btn-secondary" data-dismiss="#surat-diskusi-surat-ajax-modal" data-target="#surat-diskusi-surat-ajax-modal" onclick="$('#surat-diskusi-surat-ajax-modal').modal('hide');">Tutup</button>
                                        </div> --}}
                                </td>
                            </tr>
                        </tfoot>
...
$(function () {
    ...
    surat_diskusi_surat_table = $('#surat-diskusi-surat-data-table').on('xhr.dt', function (e, settings, json, xhr) {
        ...
        searching: false,
        info: false,
        ajax: "{{ route('diskusi_surat.index') }}",
        columns: [
            {
                data: null,
    ...
    $('body').on('click', '.surat-diskusi-surat-edit', function () {
        ...
        reset_form_data('#surat-diskusi-surat-form');
        $.get('{{ route('diskusi_surat.index') }}' + '/' + surat_diskusi_surat_id  + '/edit', function (data) {
    ...
    $('body').on('click', '.surat-diskusi-surat-delete', function (){
        ...
        if(result){
            $.ajax({
                type: "DELETE",
                url: '{{ route('diskusi_surat.store') }}' + '/' + surat_diskusi_surat_id,
                success: function (data) {
                    surat_diskusi_surat_table.ajax.url( '{{ route( 'diskusi_surat.index' ) }}?id_surat_masuk=' + surat_masuk_id ).load();
                },
                error: function (data) {
